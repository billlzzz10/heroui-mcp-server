...
export const inputSchema = z.object({
  name: z.string().optional().describe("Optional name for this use-real-shape hook variant."),
  props: z.record(z.string(), z.any()).describe("A key-value pair of props specific to this use-real-shape hook variant."),
  metadata: z.object({
  ...
  }).partial().describe("Additional metadata for the use-real-shape hook variant."),
});
...
export const HERO_HOOK_USE_REAL_SHAPE_01: Tool = {
  id: "HERO_HOOK_USE_REAL_SHAPE_01",
  name: "Define Use Real Shape Hook Variant",
  description: "Defines and stores a new use-real-shape hook variant based on provided specifications.",
  ...
  execute: async (input: z.infer<typeof inputSchema>) => {
    const newComponentVariant: Omit<ComponentVariantObject, 'hash'> = {
      ...
      targetComponent: "use-real-shape", // Using targetComponent for now, might need a new field for hooks
      name: input.name,
      props: input.props,
      styleObjectHashes: {},
