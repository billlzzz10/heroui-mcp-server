...
export const inputSchema = z.object({
  name: z.string().optional().describe("Optional name for this use-data-scroll-overflow hook variant."),
  props: z.record(z.string(), z.any()).describe("A key-value pair of props specific to this use-data-scroll-overflow hook variant."),
  metadata: z.object({
  ...
  }).partial().describe("Additional metadata for the use-data-scroll-overflow hook variant."),
});
...
export const HERO_HOOK_USE_DATA_SCROLL_OVERFLOW_01: Tool = {
  id: "HERO_HOOK_USE_DATA_SCROLL_OVERFLOW_01",
  name: "Define Use Data Scroll Overflow Hook Variant",
  description: "Defines and stores a new use-data-scroll-overflow hook variant based on provided specifications.",
  ...
  execute: async (input: z.infer<typeof inputSchema>) => {
    const newComponentVariant: Omit<ComponentVariantObject, 'hash'> = {
      ...
      targetComponent: "use-data-scroll-overflow", // Using targetComponent for now, might need a new field for hooks
      name: input.name,
      props: input.props,
      styleObjectHashes: {},
