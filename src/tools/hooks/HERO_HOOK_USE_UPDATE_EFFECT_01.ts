...
export const inputSchema = z.object({
  name: z.string().optional().describe("Optional name for this use-update-effect hook variant."),
  props: z.record(z.string(), z.any()).describe("A key-value pair of props specific to this use-update-effect hook variant."),
  metadata: z.object({
  ...
  }).partial().describe("Additional metadata for the use-update-effect hook variant."),
});
...
export const HERO_HOOK_USE_UPDATE_EFFECT_01: Tool = {
  id: "HERO_HOOK_USE_UPDATE_EFFECT_01",
  name: "Define Use Update Effect Hook Variant",
  description: "Defines and stores a new use-update-effect hook variant based on provided specifications.",
  ...
  execute: async (input: z.infer<typeof inputSchema>) => {
    const newComponentVariant: Omit<ComponentVariantObject, 'hash'> = {
      ...
      targetComponent: "use-update-effect", // Using targetComponent for now, might need a new field for hooks
      name: input.name,
      props: input.props,
      styleObjectHashes: {},
