...
export const inputSchema = z.object({
  name: z.string().optional().describe("Optional name for this use-ssr hook variant."),
  props: z.record(z.string(), z.any()).describe("A key-value pair of props specific to this use-ssr hook variant."),
  metadata: z.object({
  ...
  }).partial().describe("Additional metadata for the use-ssr hook variant."),
});
...
export const HERO_HOOK_USE_SSR_01: Tool = {
  id: "HERO_HOOK_USE_SSR_01",
  name: "Define Use SSR Hook Variant",
  description: "Defines and stores a new use-ssr hook variant based on provided specifications.",
  ...
  execute: async (input: z.infer<typeof inputSchema>) => {
    const newComponentVariant: Omit<ComponentVariantObject, 'hash'> = {
      ...
      targetComponent: "use-ssr", // Using targetComponent for now, might need a new field for hooks
      name: input.name,
      props: input.props,
      styleObjectHashes: {},
