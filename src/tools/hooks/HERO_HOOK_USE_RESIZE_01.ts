...
export const inputSchema = z.object({
  name: z.string().optional().describe("Optional name for this use-resize hook variant."),
  props: z.record(z.string(), z.any()).describe("A key-value pair of props specific to this use-resize hook variant."),
  metadata: z.object({
  ...
  }).partial().describe("Additional metadata for the use-resize hook variant."),
});
...
export const HERO_HOOK_USE_RESIZE_01: Tool = {
  id: "HERO_HOOK_USE_RESIZE_01",
  name: "Define Use Resize Hook Variant",
  description: "Defines and stores a new use-resize hook variant based on provided specifications.",
  ...
  execute: async (input: z.infer<typeof inputSchema>) => {
    const newComponentVariant: Omit<ComponentVariantObject, 'hash'> = {
      ...
      targetComponent: "use-resize", // Using targetComponent for now, might need a new field for hooks
      name: input.name,
      props: input.props,
      styleObjectHashes: {},
